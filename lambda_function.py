...
def validate_age_investment(age, investment_amount):
    if age is not None: 
        age = parse_int(age)
        if age < 0:  
            return build_validation_result(False, "age", "Your age  must be a positive number")
        elif age >= 65: 
    ...
    if investment_amount is not None:
        ...
        if investment_amount < 5000: 
           return build_validation_result(False, "investmentAmount", "The minimum to invest is $5000.")
...
def recommend_portfolio(intent_request):
    ...
    if source == 'DialogCodeHook':
        ...
        validation_result = validate_age_investment(age, investment_amount)
        ...
        return delegate(output, get_slots(intent_request))
        
    recommendation = (((risk_level.lower() == 'none') * "100% bonds (AGG), 0% equities (SPY)") +
                        ((risk_level.lower() == 'low') * "60% bonds (AGG), 40% equities (SPY)") +
                        ((risk_level.lower() == 'medium') * "40% bonds (AGG), 60% equities (SPY)") +
                        ((risk_level.lower() == 'high') * "20% bonds (AGG), 80% equities (SPY)"))
    return close(
        intent_request['sessionAttributes'],
        'Fulfilled',
        {"contentType":"PlainText", "content": f"Thank you for your information, based on the risk level you defined, my recommendation is {recommendation}"})
